...
void cDataBase_client::computePacketToNormalize(std::string RPC_PACK)
{

	int keyMarkerServerID;

	std::string paket(RPC_PACK);

	for (int i = 0; i < paket.length(); i++)
		if (paket[i] == '.') paket[i] = ',';

	std::cout << "new paket " << paket << "\n";
	//	тут будут пропарсеная инфа
	std::shared_ptr<std::list <float>> sharedLIst;						//	тут объявления указателя
	sharedLIst = std::make_shared<std::list <float>>();				/* конструктор для типа через умный ptr */

	// умеет получать числа: ( отрицательные ) целые, дробные
	boost::regex reg("(-+|)\\d+(,\\d+)*");
		...
		it != boost::sregex_iterator(); ++it)
	{
		...
		float gloablRez = (float)atof(input);

		//std::cout << "float = " << gloablRez << "\n";
		// ложим пропарсенные параметры в список
		sharedLIst->push_back(gloablRez);
	}


	std::list<float>::iterator it = sharedLIst->begin();				// итератор на начало списка


	/*for (it = sharedLIst->begin(); it != sharedLIst->end(); it++)
	{
	std::cout << "cDataBase_client::computePacketToNormalize uNbOX " << *it << "\n";
	}
	*/
	...
	if (!sharedLIst->empty())											// if список не пуст
	{
		...
		case 6: //
			...
			{
				//PLAYER_POS(byHost, design, markerServerID, x, y, z); //
				//мне пришло от сервера "65535, 33, 0, 12.563, 7645.46, 4564.4";

				// напрямую работаем с m_CLEO_Marker_map
				// и так, первым делом смотрим на серверИД
				// если его нет то формируем стек с флагом для того что бы его
				// прочел клео скрипт
				// а если он есть то ?

				std::cout << "мне пришла новая позиция маркера на карте (ОСТАВШИЕСЯ ПАРАМЕТРЫ)" << "\n";

				/*	while (it != sharedLIst->end()) // этот вайл двигает каретку указателя лол
					{
					std::cout << " (float) " << *it << "\n";
					it++;
					}*/
				...
				it++;
				//markerServerID_POSSITIONS (byHost, design, markerServerID, x, y, z);
				//													^

				keyMarkerServerID = (int)(*it);


				/*int keyMarkerServerID = (int)(*it);

				auto itSearch = std::find_if(m_CLEO_Marker_map.begin(), m_CLEO_Marker_map.end(),
					[&keyMarkerServerID](const std::pair<int, struct ClientMarker> &p)
				{
					return p.first == keyMarkerServerID;
				});
*/
				// в любом случии обновить структуру маркеров ?


				 
				//if (itSearch != m_CLEO_Marker_map.end())
				//{												// такой ServerMarker_ID уже Е
				//	
				//std::cout << "refresh1" << "\n";
				/*it++;
				std::cout << "обновть позицию Х:" << (*it) << "\n";
				(*itSearch).second.x = (float)(*it);

				it++;
				std::cout << "обновть позицию Y:" << (*it) << "\n";
				(*itSearch).second.y = (float)(*it);

				it++;
				std::cout << "обновть позицию z:" << (*it) << "\n";
				(*itSearch).second.z = (float)(*it);*/
				//}
				//else
				//{															// register

				//	std::cout << "register" << "\n";
				//ClientMarker tmpforBox;

				//it++;
				//std::cout << "зарегистрировать позицию Х:" << (*it) << "\n";
				//tmpforBox.x = (float)(*it);

				//it++;
				//std::cout << "зарегистрировать позицию Y:" << (*it) << "\n";
				//tmpforBox.y = (float)(*it);

				//it++;
				//std::cout << "зарегистрировать позицию z:" << (*it) << "\n";
				//tmpforBox.z = (float)(*it);


				//m_CLEO_Marker_map.insert(std::pair<int, struct ClientMarker>(keyMarkerServerID, tmpforBox));

				//// и так мы распарсили пакет и положили в локальную БД а теперь его нужно положить в
				//// стек для клео, С Флагом спавна!!!

				//std::cout << "незабыть дописать фичу  стека для клео" << "\n";
		...
		default: std::cout << "AHTUNG switch po size() = " << sharedLIst->size() << "\n";  break;
...
void	сUDPClientGTA::recov()
{
	...
	while (true)
	{
		...
		m_DataBasa->computePacketToNormalize(msg);
	}
}
